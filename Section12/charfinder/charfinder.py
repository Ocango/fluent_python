...
class UnicodeNameIndex:
    ...
    def load(self, chars=None):
        ...
        if chars is None:
            try:
                with open(INDEX_NAME, 'rb') as fp:#加载本地序列化存储内容
    ...
    def find_chars(self, query, start=0, stop=None):
        ...
        result = functools.reduce(set.intersection, result_sets)#返回集合共有元素
        result = sorted(result)  # must sort to support start, stop
        result_iter = itertools.islice(result, start, stop)#切片
